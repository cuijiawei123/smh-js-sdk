...
| withInode | 是否返回 inode（文件目录 ID），0 或 1，默认不返回 | Number | 否 |
| userId | 用户身份识别 | String | 否 |
| createDirectoryRequest | 可选的请求体，用于指定目录的元数据信息。如果不需要设置元数据，请求体可以为空 | Object | 否 |
...
| labels | 目录标签列表 | Array | 否 |
| localCreationTime | 目录对应的本地创建时间，时间戳字符串 | String | 否 |
| localModificationTime | 目录对应的本地修改时间，时间戳字符串 | String | 否 |
